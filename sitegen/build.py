#!/usr/bin/env python3
# Generador de sitio estatico multi-categoria para monetizacion con AdSense.
...
def nav_html():
    links="".join(f'<a href="/categoria-{c["slug"]}.html">{html.escape(c["name"])}</a>' for c in CATS)
    return links
...
def category_html(c):
    ...
    canonical=f"{DOMAIN}/categoria-{c['slug']}.html"
    return page(c["name"],f"Articulos de {c['name']} en {CONFIG['brand']}.",body,canonical)
...
<p>Nuestro objetivo es explicar cada tema de forma clara y util, para ayudarte a decidir y a ahorrar tiempo. Si tienes dudas, escribenos desde la <a href="/contacto.html">pagina de contacto</a>.</p>
"""),
...
def sitemap():
    ...
    urls=[f"{DOMAIN}/"]
    urls+=[f"{DOMAIN}/categoria-{c['slug']}.html" for c in CATS]
    urls+=[f"{DOMAIN}/{a['slug']}.html" for a in ARTICLES]
    urls+=[f"{DOMAIN}/{n}" for n in LEGAL]
